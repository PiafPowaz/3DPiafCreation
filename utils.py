...
def makeSquarre(color, middlehight = WINDOWWIDTH/2, middlewheight = WINDOWWHEIGHT/2, w = 32, h = 32, top = -1, left = -1):
    squarreSurf = pygame.Surface((int(w),int(h)))
    squarreRect = squarreSurf.fill(color)
    if top != -1 or left != -1:
		if top != -1:
			squarreRect.top = top
		if left != -1:
			squarreRect.left = left
    else:
...
class Point(Image):
    def __init__(self, color = (0,0,0), x = 0, y = 0, size = 2):
        ...
        self.w = size
        self.contour = False
        self.surf, self.rect = makeSquarre(color, w = self.w-1*self.contour, h = self.h-1*self.contour, middlehight = x, middlewheight = y )
        if self.contour:
            self.surf_contour, self.rect_contour = makeSquarre((0,0,0), h = self.h, w = self.w, middlehight = x, middlewheight = y )
        self.x = x
        self.y = y
